...
namespace TomatoPHP\TomatoThemes\Services;
...
class Theme
{
    ...
    public function build(): void
    {
        foreach ($this->sections as $section) {
            $checkIfSectionExists = \TomatoPHP\TomatoThemes\Models\Section::where('key', $section->key)->first();
            if(!$checkIfSectionExists){
                $form = Form::where('key', $section->form)->first();
                $section->form_id($form->id);
                \TomatoPHP\TomatoThemes\Models\Section::create($section->toArray());
            }
        }
    }
}
